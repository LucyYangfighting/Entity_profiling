...
public class relation_cosine_similarity {
    ...
    //The label type is relation:Calculate the similarity between positive classes
    private static void relation_positive_sim(Connection db) throws Exception{
        try (Statement stmt = db.createStatement()) {
            //创建relation_positive_sim表: 节点相似度
            stmt.execute("CREATE TABLE relation_positive_sim (entity_1_type INTEGER NOT NULL,predicate_id  INTEGER NOT NULL," +
                    "entity2_id INTEGER NOT NULL, similarity DOUBLE NOT NULL);");
        }
        try (Statement indexingStmt = db.createStatement()) {
            indexingStmt.execute("CREATE INDEX en1_type ON relation_positive_sim(entity_1_type)");
        }
        db.commit();
        ...
        try(
                ...
                PreparedStatement relations_Stmt = db.prepareStatement("SELECT DISTINCT predicate_id, entity2_id, entity1_type_id FROM salience; ");
                PreparedStatement en_vec_Stmt = db.prepareStatement("SELECT entity_id, vector FROM linkedmdb_vec; ");
                PreparedStatement insert_relation_sim_Stmt = db.prepareStatement("INSERT INTO relation_positive_sim " +
    ...
    //The label type is relation:Calculate the similarity between positive classes and negative classes
    private static void relation_negative_sim(Connection db) throws Exception{
        try (Statement stmt = db.createStatement()) {
          
            stmt.execute("CREATE TABLE relation_negative_sim (entity_1_type INTEGER NOT NULL,predicate_id  INTEGER NOT NULL," +
                    "entity2_id INTEGER NOT NULL, similarity DOUBLE NOT NULL);");
        }
        try (Statement indexingStmt = db.createStatement()) {
            indexingStmt.execute("CREATE INDEX en1_type_1 ON relation_negative_sim(entity_1_type)");
        }
        db.commit();
        Map<Integer, double[]> vectorMap = new HashMap<Integer, double[]>();
        ...
        try(
                ...
                PreparedStatement en_vec_Stmt = db.prepareStatement("SELECT entity_id, vector FROM linkedmdb_vec; ");
                PreparedStatement insert_relation_sim_Stmt = db.prepareStatement("INSERT INTO relation_negative_sim " +
                        "(entity_1_type,predicate_id,entity2_id,similarity) VALUES (?,?,?,?);")
        )
        {
            ResultSet rel_str = relation_Stmt.executeQuery();
    ...
    //The label type is relation_property_string:Calculate the similarity
   private static void rel_pro_str_sim(Connection db) throws Exception{
        try (Statement stmt = db.createStatement()) {
            
            stmt.execute("CREATE TABLE relation_string_sim (entity1_type INT NOT NULL ,predicate_id INT,property_id INT ,property_value INT," +
                    "s_s_sim DOUBLE NOT NULL, s_w_sim DOUBLE NOT NULL,s_c_sim DOUBLE NOT NULL);");
        }
        try (Statement indexingStmt = db.createStatement()) {
            indexingStmt.execute("CREATE INDEX e_type ON relation_string_sim(entity1_type)");
        }
        db.commit();
        ...
        try(    PreparedStatement relation_pro_Stmt = db.prepareStatement("SELECT DISTINCT entity1_type_id, entity1_id,predicate_id,entity2_id, property_id,property_value FROM relation_property_str; ");
                PreparedStatement re_pro_Stmt = db.prepareStatement("SELECT DISTINCT entity1_type_id,predicate_id,property_id,property_value FROM relation_property_str; ");
                PreparedStatement entity_negative_Stmt = db.prepareStatement("SELECT node_id, type_id  FROM nodes_type ; ");
                PreparedStatement en_vec_Stmt = db.prepareStatement("SELECT entity_id, vector FROM linkedmdb_vec; ");
                PreparedStatement insert_relation_sim_Stmt = db.prepareStatement("INSERT INTO relation_string_sim " +
        ...
        )
        {
            ...
            while(relation_pro_str.next()){
                ...
                if (weak_node_set.size()==0 && node_negetive_set.size()==0){
                    //the  entity is all strong_positive entity,no weak-positive entity,no negative entity
    ...
    //The label type is relation_property_numerical:Calculate the similarity 
    private static void rel_pro_num_sim(Connection db) throws Exception{
        try (Statement stmt = db.createStatement()) {
           
            stmt.execute("CREATE TABLE relation_numerical_sim (entity1_type INT NOT NULL ,predicate_id INT,property_id INT ,property_value_range CHAR ," +
                    "s_s_sim DOUBLE NOT NULL, s_w_sim DOUBLE NOT NULL,s_c_sim DOUBLE NOT NULL);");
        }
        try (Statement indexingStmt = db.createStatement()) {
            indexingStmt.execute("CREATE INDEX rela_pro_num_e_type ON relation_numerical_sim(entity1_type)");
        }
        db.commit();
        ...
        Map<ArrayList<Integer>, Integer> en_n_Map = new HashMap<ArrayList<Integer>, Integer>();
        try(    PreparedStatement relation_pro_Stmt = db.prepareStatement("SELECT DISTINCT entity1_type, entity1_id,predicate_id,entity2_id, property_id,property_value_range FROM relation_property_num; ");
                PreparedStatement re_pro_Stmt = db.prepareStatement("SELECT DISTINCT entity1_type,predicate_id,property_id,property_value_range FROM relation_property_num; ");
                PreparedStatement entity_negative_Stmt = db.prepareStatement("SELECT node_id, type_id  FROM nodes_type ; ");
                PreparedStatement en_vec_Stmt = db.prepareStatement("SELECT entity_id, vector FROM linkedmdb_vec; ");
                PreparedStatement insert_relation_sim_Stmt = db.prepareStatement("INSERT INTO relation_numerical_sim " +
    ...
    //The label type is relation:Similarity between positive classes minus similarity between positive and negative classes
    private static void relation_similarity(Connection db) throws Exception{
        
        try(Statement stmt = db.createStatement()){
            stmt.execute("CREATE TABLE relation_similarity (entity_1_type INTEGER NOT NULL,predicate_id  INTEGER NOT NULL," +
                    "entity2_id INTEGER NOT NULL, similarity DOUBLE NOT NULL)");
            stmt.execute("CREATE INDEX rel_sim_dif ON relation_similarity(similarity)");
        }
        db.commit();
        try(PreparedStatement stmt_1 = db.prepareStatement("SELECT entity_1_type,predicate_id,entity2_id, similarity FROM relation_positive_sim");
            PreparedStatement stmt_2 = db.prepareStatement("SELECT similarity FROM relation_negative_sim WHERE entity_1_type=? AND predicate_id=? AND entity2_id=?");
            PreparedStatement stmt_3 = db.prepareStatement("INSERT INTO relation_similarity(entity_1_type,predicate_id,entity2_id,similarity) VALUES(?,?,?,?);")
    ...
    //The label type is relation_property_string:Count the difference between  strong label,  weak label, and  negative label
    ...
    //The label type is relation_property_numerical:Count the difference between  strong label,  weak label, and  negative label
    ...
    public static void main(String[] args)throws Exception {
        ...
        rel_pro_str_sim(db);
        rel_pro_num_sim(db);
        relation_string_similarity(db);
        relation_numerical_similarity(db);
